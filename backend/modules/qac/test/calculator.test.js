...
test('integracja: osie kątowe + Pars Fortunae w formie świadomej (wymaga efemeryd)', (t) => {
    ...
    // Gdańsk, 14.01.1955, 07:20 UTC (fixture syntetyczny)
    const wynik = obliczDaneSurowe({
        czas_utc: { rok: 1955, miesiac: 1, dzien: 14, godzina: 7, minuta: 20, sekunda: 0 },
...
test('pozycje: zodiak sideralny (Lahiri) — pole sideralna, ayanamsa ≈ 23,23° dla 1955 (wymaga efemeryd)', (t) => {
    if (!efemerydyDostepne) {
        t.skip('POMINIĘTO — pliki efemeryd niedostarczone (ephemeris/README.md)');
        return;
    }
    // Gdańsk, 14.01.1955, 07:20 UTC (fixture syntetyczny) — obie formy liczone wspólną pętlą po CIALA.
    const wynik = obliczDaneSurowe({
        czas_utc: { rok: 1955, miesiac: 1, dzien: 14, godzina: 7, minuta: 20, sekunda: 0 },
        obserwator: { dlugosc_geo: 18.6466, szerokosc_geo: 54.3520, wysokosc_npm_m: 6 },
    });
    // get_ayanamsa(jd_et 1955) = 23,234° (Lahiri). Różnica tropikalna−sideralna
    // to ayanamsa — stała dla wszystkich ciał w danej chwili.
    const AYANAMSA_LAHIRI_1955 = 23.23;
    ...
    for (const forma of ['forma_swiadoma', 'forma_nieswiadoma']) {
        ...
        for (const [nazwa, p] of Object.entries(pozycje)) {
            ...
            assert.ok(Math.abs(ayanamsa - AYANAMSA_LAHIRI_1955) < 0.1,
                `${forma}.${nazwa}: tropikalna−sideralna=${ayanamsa.toFixed(4)}° poza ${AYANAMSA_LAHIRI_1955}±0.1`);
        }
    }
});
...
test('nakszatry: mapa dla wszystkich obiektów formy świadomej, Księżyc → 12/0 (wymaga efemeryd)', (t) => {
    ...
    // Kluczowa asercja: Księżyc sideralnie ≈161,89° → nakszatra 12, pada 0.
...
test('kwantyzacja E2E: Słońce natalne (tropikalne) → brama 61 koła rave (wymaga efemeryd)', (t) => {
    ...
    // Kluczowa asercja: Słońce 293,35° tropikalne → brama 61.
...
test('czas lokalny→UTC: czas zimowy w Warszawie (CET, +1 h)', () => {
    const wynik = lokalnyNaUtc(
        { rok: 1958, miesiac: 12, dzien: 3, godzina: 1, minuta: 10, sekunda: 0 },
        'Europe/Warsaw'
    );
    assert.deepEqual(wynik.czas_utc, {
        rok: 1958, miesiac: 12, dzien: 3, godzina: 0, minuta: 10, sekunda: 0,
    });
...
test('czas lokalny→UTC: historyczna reguła sprzed reformy (1955)', () => {
    const wynik = lokalnyNaUtc(
        { rok: 1955, miesiac: 1, dzien: 14, godzina: 8, minuta: 20, sekunda: 0 },
        'Europe/Warsaw'
    );
    assert.deepEqual(wynik.czas_utc, {
        rok: 1955, miesiac: 1, dzien: 14, godzina: 7, minuta: 20, sekunda: 0,
    });
    assert.equal(wynik.offset_minuty, 60);
});
...
test('czas lokalny→UTC: przejście przez północ cofa datę', () => {
    const wynik = lokalnyNaUtc(
        { rok: 1958, miesiac: 12, dzien: 3, godzina: 0, minuta: 30, sekunda: 0 },
        'Europe/Warsaw'
    );
    assert.deepEqual(wynik.czas_utc, {
        rok: 1958, miesiac: 12, dzien: 2, godzina: 23, minuta: 30, sekunda: 0,
    });
});
